...
class segmenthead(nn.Module):
    def __init__(self, inplanes, interplanes, outplanes):
        ...
        self.conv1 = nn.Conv3d(inplanes, interplanes, kernel_size=3, padding=1, bias=True)
        self.bn2 = nn.BatchNorm3d(interplanes, momentum=bn_mom)
        self.conv2 = nn.ConvTranspose3d(interplanes , interplanes, kernel_size=3, padding=1, stride=2, output_padding=1, bias=True)
        self.bn3 = nn.BatchNorm3d(interplanes, momentum=bn_mom)
        self.conv3 = nn.Conv3d(interplanes, interplanes, kernel_size=3, padding=1, bias=True)
        self.bn4 = nn.BatchNorm3d(interplanes, momentum=bn_mom)
        self.conv4 = nn.ConvTranspose3d(interplanes, interplanes, kernel_size=3, padding=1, stride=2, output_padding=1, bias=True)
        # self.conv4 = nn.ConvTranspose3d(interplanes, interplanes, kernel_size=3, padding=1, stride=1, output_padding=0, bias=True)
        self.bn5 = nn.BatchNorm3d(interplanes, momentum=bn_mom)
        self.conv5 = nn.Conv3d(interplanes, interplanes//2, kernel_size=3, padding=1, bias=True)
        self.bn6 = nn.BatchNorm3d(interplanes//2, momentum=bn_mom)
        self.conv6 = nn.Conv3d(interplanes//2, outplanes, kernel_size=3, padding=1, bias=True)
        self.relu = nn.ReLU(inplace=True)
...
class Selfattention(nn.Module):
    ...
    def forward(self, x_q, x_k, x_v):
        ...
        x = torch.einsum('bhlt,bhtv->bhlv', [attn, v]) # B*H*N1*d
...
class RATNet(nn.Module):
    def __init__(self, block, layers, num_classes=2, dim_low=64, dim_high=32, dim_head=32, augment=False):
        ...
        self.conv1 =  nn.Sequential(
                          nn.Conv3d(1,dim_high,kernel_size=3, stride=2, padding=1),
                          nn.BatchNorm3d(dim_high, momentum=bn_mom),
                          nn.ReLU(inplace=True),
                          nn.Conv3d(dim_high,dim_high,kernel_size=3, padding=1),
                          nn.BatchNorm3d(dim_high, momentum=bn_mom),
                          nn.ReLU(inplace=True),
                          nn.Conv3d(dim_high,dim_high,kernel_size=3, stride=2, padding=1),
                        #   nn.Conv3d(dim_high,dim_high,kernel_size=3, stride=1, padding=1),
                          nn.BatchNorm3d(dim_high, momentum=bn_mom),
                          nn.ReLU(inplace=True),
                          nn.Conv3d(dim_high,dim_high,kernel_size=3, padding=1),
                          nn.BatchNorm3d(dim_high, momentum=bn_mom),
        ...
        self.down1 = nn.Sequential(nn.Conv3d(dim_high, dim_low, kernel_size=3, stride=2, padding=1, bias=False),
                                   nn.BatchNorm3d(dim_low, momentum=bn_mom),
                                   nn.ReLU(inplace=True))
        self.down2 = nn.Sequential(nn.Conv3d(dim_high, dim_low, kernel_size=3, stride=2, padding=1, bias=False),
                                   nn.BatchNorm3d(dim_low, momentum=bn_mom),
                                   nn.ReLU(inplace=True))
        self.down3 = nn.Sequential(nn.Conv3d(dim_high, dim_low, kernel_size=3, stride=2, padding=1, bias=False),
                                   nn.BatchNorm3d(dim_low, momentum=bn_mom),
                                   nn.ReLU(inplace=True))           
    ...
    def encode(self, x):
        ...
        out = self.Transformero(torch.cat(skips, dim=1), torch.cat(skips, dim=1))
        return out
...
def get_model():
    return RATNet('basic', [2, 2, 2, 2])
    
if __name__ == '__main__':
    import os
    os.environ["CUDA_VISIBLE_DEVICES"]="0"
    device = torch.device("cuda")
    model = RATNet('basic', [2, 2, 2, 2, 2]).to(device)
